...
# Обновление и переключение на storage_1c
git checkout -B "storage_1c" "origin/storage_1c" || { echo "Failed to checkout storage_1c"; exit 1; }
git pull || { echo "Failed to pull storage_1c"; exit 1; }

# Переключение на branch_sync_hran
git checkout -B "branch_sync_hran" "origin/branch_sync_hran" || { echo "Failed to checkout branch_sync_hran"; exit 1; }
